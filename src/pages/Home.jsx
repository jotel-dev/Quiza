...
function GlassCard({ children, className = "", whileHover, ...props }) {
  return (
    <motion.div 
  ...
  return (
    <div className="flex-1 p-4 sm:p-6 overflow-y-auto">
              ...
              {showWalletMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-100 rounded-xl shadow-lg py-1 z-50">
                  <motion.button
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-red-600 font-semibold transition"
                  >
                    Disconnect Wallet
                  </motion.button>
                </div>
      ...
      >
        <GlassCard 
          variants={itemVariants}
          className="p-8 sm:p-12 bg-gradient-to-br from-indigo-50/80 via-white to-orange-50/40 relative overflow-hidden w-full"
        >
          <div className="relative z-10 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-8">
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                onClick={onStartQuiz}
                className="flex items-center gap-2 bg-[#4F46E5] text-white text-base font-semibold px-6 py-3 rounded-xl shadow-lg shadow-indigo-200 transition"
              >
                <Play size={18} fill="white" />
                Start Quiz
              </motion.button>
              <motion.button 
                whileHover={isDailyChallengePlayed ? {} : { scale: 1.02, y: -2 }}
                whileTap={isDailyChallengePlayed ? {} : { scale: 0.96 }}
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-2">
          
          {/* Weekly Progress */}
          <GlassCard variants={itemVariants} whileHover={{ y: -4, boxShadow: "0 20px 40px -10px rgba(79,70,229,0.15)" }} className="p-6 sm:p-8 flex flex-col justify-between">
          ...
          {/* Recent Activity */}
          <GlassCard variants={itemVariants} whileHover={{ y: -4, boxShadow: "0 20px 40px -10px rgba(79,70,229,0.15)" }} className="p-6 sm:p-8">
